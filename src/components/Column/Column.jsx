    ...
    return (
        <div>
            ...
            <div
                ...
                <ColumnHeader badgeTitle={title} color={colors.black.badge} taskNumber={tasks.length} id={id}
                              setOpen={setModal}/>
                {tasks.map(task => <Task key={task.id} tags={task.tags} title={task.text} desc={task.description} makers={task.makers} id={task.id} columnId={id} refetch={refetch}/>)}
            </div>
        </div>
    );
};
