...
import {addUserToTask, deleteTask} from "../../../API/Service";
...
const AddUserTaskModal = ({makers, taskId, refetch}) => {
    ...
    const mutation = useMutation(["add-user-to-task", userId, boardId, taskId], (memberId) => addUserToTask(userId, boardId, taskId, memberId), {
        onSuccess: () => {
            refetch()
        }
    })
    ...
    return (
        <div className="relative mx-auto my-20 bg-white rounded-2xl shadow-2xl w-96">
            <div className="px-6 py-6 lg:px-8">
                <h3 className="mb-4 text-xl font-bold text-gray-900">Add task members</h3>
                <div className="space-y-6">
                    <div>
                        <label className="block mb-2 text-sm font-medium text-gray-900">
                            All board members </label>
                        <div className='scrollbar flex flex-col h-40 overflow-y-auto overflow-x-hidden'>
                            {users.length === 0 ?
                                <span className='font-light text-sm text-zinc-400'>No members</span> : users.map(user => {
                                    if (makersId.find(makerId => makerId === user.id) === undefined) {
                                        return <div>
